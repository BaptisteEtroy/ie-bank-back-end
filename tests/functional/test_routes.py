...
def test_create_account(testing_client):
    ...
    THEN check the response is valid
    """
    response = testing_client.post('/accounts', json={'name': 'John Doe', 'currency': '€', 'country' : 'UK'})
    assert response.status_code == 200
...
def test_update_account(testing_client):
    ...
    """
    testing_client.post(
        "/accounts", json={"name": "John Doe", "country": "UK", "currency": "€"}
    )  
    response = testing_client.put("/accounts/1", json={"name": "Baptiste"})
    assert response.status_code == 200
